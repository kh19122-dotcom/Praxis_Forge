...
def test_fault_configure_survives_immediate_post_replace_restart(tmp_path: Path) -> None:
    ...
    after = Store(Settings(seed="obj-002", state_path=path))
...
def test_markerless_trace_highwater_rejected_rolled_equals_valid_history(tmp_path: Path) -> None:
    path = str(tmp_path / "pvs.json")
    first = Store(Settings(seed="obj-002", state_path=path))
    created = first.create_task("markerless-hw-0001", "synth-ada", "synth-task", "normal")
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    ...
    assert not hasattr(Store, "next_trace_id")
    follow = restored.create_task("markerless-hw-0002", "synth-ben", "synth-other", "high")
