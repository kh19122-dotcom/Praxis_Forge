...
class Store:
    ...
    def _restore_locked(self) -> bool:
        ...
        bookings, bookings_by_key, events, seq, trace, slot_booking_ids, epoch = validated
...
def _validate_trace_identities(events: list[dict]) -> None:
    ...
    for group in grouped.values():
        ...
        for event in group:
            ...
            if event_type == "booking_requested":
                requested += 1
            if event_type in {
                "booking_committed",
                "booking_replayed",
            ...
            if event_type in {"booking_committed", "booking_replayed"}:
                booking_id_value = details.get("booking_id")
                if isinstance(booking_id_value, str):
                    object_ids.add(booking_id_value)
            if event_type in {"booking_requested", "booking_committed", "booking_replayed"}:
