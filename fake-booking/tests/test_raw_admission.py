...
def test_overlapping_resets_serialize_and_preserve_new_create() -> None:
    ...
    try:
        ...
        old_sock = _send_partial_post(
            ...
            port,
            "/v1/bookings",
            {"Idempotency-Key": "raw-old-overlap-booking"},
            old_body,
            first_bytes,
        )
        try:
            ...
            def _new_post() -> None:
                with httpx.Client(timeout=5.0) as client:
                    response = client.post(
                        f"{base}/v1/bookings",
                        headers={"Idempotency-Key": "raw-new-overlap-booking"},
                        json={"slot_id": slot["id"], "patient_ref": "synth-ada"},
                    )
                    late["new_status"] = response.status_code
                    late["new_body"] = response.json()
            ...
            assert old_body_json["error"] == "epoch_stale"
            assert late["new_status"] == 201
            booking_id = late["new_body"]["id"]
        finally:
            old_sock.close()

        with httpx.Client(timeout=2.0) as client:
            fetched = client.get(f"{base}/v1/bookings/{booking_id}")
